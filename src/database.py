...
class DataBase:
    ...
    def loadContact(self) -> list:
        try:
            self.cursor.execute("SELECT rowid, * FROM contacts")
    ...
    def create_contact(self, name, phone_num, email, addr):
        if (name and phone_num and email and addr) and not self.contact_detail(
            name, phone_num
        ):
    ...
    def search(self, name="", phone_num=""):
        try:
            ...
            self.cursor.execute(
                "SELECT rowid,* FROM contacts WHERE name LIKE ? AND phone_num LIKE ?",
                (
                    modified_name,
                    modified_num,
                ),
